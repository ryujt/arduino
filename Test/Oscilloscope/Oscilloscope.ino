...
void setup() {            
  ...
  attachInterrupt( pin_int-2, interrupt_handler, FALLING );
}
...
void loop() {
  ...
  int data_in = analogRead( A0 );
  
  value_sum = value_sum + data_in;  
//  if (data_in >= 500) value_sum++;
  value_count++;
...
//    value = value_sum * 1000 / value_count;
//    value = data_in;
